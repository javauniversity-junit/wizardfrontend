...
public class WebClientController {
	...
	@GetMapping(path = "/detailClient") // Map ONLY GET Requests
	@ResponseBody
	public String detail(HttpServletResponse response, @RequestParam(name = "ID", required = true) Integer id, Model model) {
		WebClient client = webClientRepository.findById(id).get();
		User user  = userRepository.findById(client.getAuthorId()).get();
		response.addHeader("authorName", user.getName());
		Category category  = categoryRepository.findById(client.getCategoryId()).get();
		response.addHeader("categoryName", category.getName());
	...
	@ResponseBody
	public String save(HttpServletResponse response, @RequestParam(name ="name", required=true) String name
		...
		webClient.setAuthorId(authorId);
		Integer categoryId = Integer.parseInt(category);
		webClient.setCategoryId(categoryId);
		webClient.setUrl(url);
		webClient.setRequest(requestmessage);
		webClient.setDescription(description);
		webClient.setName(name);
		//determine if update needed
		if (id != null && !id.isEmpty()) {
			Integer webclientId = Integer.parseInt(id);
			webClient.setId(webclientId);
		}
		...
		//set date executed
		String timeStamp = new SimpleDateFormat("EEEE, dd MMMM yyyy, hh:mm:ss.SSS a").format(Calendar.getInstance().getTime());
		webClient.setExecutionDate(timeStamp);
		
		webClientRepository.save(webClient);
		String resultMessage = SoapManager.send(webClient);
		
		response.addHeader("id", webClient.getId().toString());
